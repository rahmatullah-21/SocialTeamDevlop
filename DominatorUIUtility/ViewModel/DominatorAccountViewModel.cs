...
namespace DominatorUIUtility.ViewModel
{
    ...
    public class DominatorAccountViewModel : BindableBase, IDominatorAccountViewModel
    {
        ...
        public bool IsSortAccounts { get; set; }
            = ServiceLocator.Current.GetInstance<ISoftwareSettingsFileManager>().GetSoftwareSettings()
                ...
                if (result == MessageDialogResult.Affirmative)
                {
                    ...
                    if (LstDominatorAccountModel.Any(x =>
                        ...
                        Dialog.ShowDialog("LangKeyNote".FromResourceDictionary(),
                            "LangIGTikTokWontRunWithBrowserAutoTryWithHttp".FromResourceDictionary());
        ...
        public void AccountBrowserLogin(DominatorAccountModel dominatorAccountModel)
        {
            try
            {
                Task.Factory.StartNew(() =>
                {
                    ...
                    var browserManager = accountScopeFactory[$"{dominatorAccountModel.AccountId}_BrowserLogin"]
                        .Resolve<IBrowserManager>(dominatorAccountModel.AccountBaseModel.AccountNetwork.ToString());
        ...
        public void DeleteAccountByContextMenu(DominatorAccountModel selectedRow)
        {
            ...
            var dialogResult = DialogCoordinator.Instance.ShowModalMessageExternal(Application.Current.MainWindow,
                ...
                Dialog.SetMetroDialogButton("LangKeyDeleteAnyway".FromResourceDictionary(),
            ...
            try
            {
                lock (_syncLoadAccounts)
                {
                    try
                    {
                        ...
                        var availablenetworks = ServiceLocator.Current.GetAllInstances<ISocialNetworkModule>()
                            .Select(y => y.Network);
        ...
        private string DefaultAccountNameFromModel(IEnumerable<DominatorAccountModel> listAcc,
            ref Dictionary<SocialNetworks, int> dictNetLasNum, SocialNetworks net)
        {
            var preName = $"{net.ToString()} Account ";
            try
        ...
        public void MultipleUpdate(DominatorAccountModel account, string updateMenuItem,
            IAccountUpdateFactory accountFactory)
        {
                        ...
                        try
                        {
                            ...
                            if (checkResult)
                            {
                                var runningActivityManager =
                                    ServiceLocator.Current.GetInstance<IRunningActivityManager>();
