...
namespace DominatorHouseCore.DatabaseHandler.Common.EntityCounters
{
    ...
    public abstract class FilterPredicate<TSource, TInput, TValue> : IFilterPredicate<TSource> where TSource : class, new()
    {
        ...
        protected virtual Expression<Func<TSource, bool>> CreateExpression(Expression<Func<TSource, TValue>> filterExpression, TValue value)
        {
    ...
    public class ActivityTypeFilterPredicate<TSource, TValue> : FilterPredicate<TSource, ActivityType, TValue> where TSource : class, new()
    {
        ...
        protected override Expression<Func<TSource, bool>> CreateExpression(Expression<Func<TSource, TValue>> filterExpression, TValue value)
        {
            return Expression.Lambda<Func<TSource, bool>>(
                Expression.Equal(
                    filterExpression.Body,
    ...
    public class ActivityTypeAsStringFilterPredicate<TSource> : ActivityTypeFilterPredicate<TSource, string> where TSource : class, new()
    {
