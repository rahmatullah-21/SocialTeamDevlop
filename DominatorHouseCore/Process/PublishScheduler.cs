           ...
           int postsMaximumDestinationCount)
        {
            ...
            lock (updatelock)
            {
                ...
                try
                {
                    ...
                    // Validate whether all destinations contains posts or not
                    if (pendingPostList.Count < postsDestinations.Count)
                        GlobusLogHelper.log.Info(Log.CustomMessage, SocialNetworks.Social, campaignName, "LangKeyPublisher".FromResourceDictionary(), "Pending postlist counts are lesser than required  random destination count count!");
